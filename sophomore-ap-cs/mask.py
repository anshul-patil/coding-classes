...
def round_corners_one_image(original_image, percent_of_side=.3):
    ...
    return result 
def change_color(img_given):
    image = img_given.convert("RGBA")
...
def round_corners_of_all_images(directory=None):
    ...
    for n in range(len(image_list)):
        ...
        new2_image = change_color(new_image)
        
        # Save the altered image, suing PNG to retain transparency
        new2_image_filename = os.path.join(new_directory, filename + '.png')
        new2_image.save(new2_image_filename)    