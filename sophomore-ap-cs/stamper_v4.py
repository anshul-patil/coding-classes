...
reduceSlider = Scale(root,variable=reduction, from_=0.5, to=0.99,
                     orient=HORIZONTAL, resolution=.01, label='Reduction:')
reduction.set(.95)
reduceSlider.grid(column=0, row=4, sticky=W)
...
# To prevent garbage collection of the images, place images in a list that 
# persists in the global context of the program (i.e., outside a def function)
canvas.imglist=[] # Create an empty list into which img references can be placed
...
def stamp(event):
    def iterate(iterations_remaining):
        if iterations_remaining>0:
            ...
            canvas.imglist.append(tkimg) # prevents garbage collection when stamp exits
